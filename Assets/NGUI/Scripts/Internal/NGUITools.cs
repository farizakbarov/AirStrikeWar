...
static public class NGUITools
{
	...
	static public AudioSource PlaySound (AudioClip clip, float volume, float pitch)
	{
		...
		if (clip != null && volume > 0.01f)
		{
			...
			{
				AudioSource source = mListener.transform.parent.GetComponent<AudioSource>();
				if (source == null) source = mListener.transform.parent.gameObject.AddComponent<AudioSource>();
				source.pitch = pitch;
