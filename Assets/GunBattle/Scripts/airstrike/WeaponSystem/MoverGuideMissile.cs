...
public class MoverGuideMissile : WeaponBase
{
	...
	private void Update ()
	{
		...
		if (Target) {
			Quaternion rotation = Quaternion.LookRotation (Target.transform.position + new Vector3(0,2,3) - transform.transform.position);
			transform.rotation = Quaternion.Slerp (transform.rotation, rotation, Time.deltaTime * Damping);
			Vector3 dir = (Target.transform.position + new Vector3(0,2,3) - transform.position).normalized;
